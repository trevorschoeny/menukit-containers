...
/**
 * The per-frame geometry handed to a {@link GraftScreenPresence}'s callbacks —
 * the current screen, its frame origin ({@code leftPos}/{@code topPos}), and the
 * cursor. Everything a consumer needs to position decoration, reposition grafted
 ...
 */
public record GraftScreenContext(
    ...
    /** The current screen's menu — where the grafted slots live (and get repositioned). */
