...
/**
 * Projects {@code InventoryMenu}-bound grafts onto <b>foreign</b> menus — chests,
 * furnaces, donkeys, any {@link AbstractContainerMenu} — so a grafted slot a
 ...
 * "every screen whose menu already carries the graft" to "every screen that shows
 * the player inventory."
 *
 * <h3>The §0019 line this deliberately crosses</h3>
 *
 * {@link MenuKitGraft} ships <b>no</b> registry on purpose: for the player's own
 ...
 * possible: a registry of graft <em>factories the library merely invokes</em>
 * (data + a deterministic build function), never a behavior callback that decides
 * interaction semantics. The grafts it builds are ordinary {@link MenuKitSlot}s, so
 * render / click→Storage / creative / sync all ride the same vanilla-slot paths
 * the {@code InventoryMenu} graft already uses — the library adds a slot type to a
 * foreign menu's list, it invents no behavior.
 *
 * <h3>Sync safety — the load-bearing contract</h3>
 *
 * The grafts must appear on the <b>server</b> menu and the <b>client</b> menu with
 ...
 * flicker/dupe). Both seams therefore call this one {@link #appendProjectedGrafts}
 ...
 *       {@code ContainerSetContent}, so that packet already includes the grafts;</li>
 ...
 *       so the client menu carries the grafts when it arrives.</li>
 * </ul>
 * Because both sides iterate the same registered sources in the same order and each
 * source's factory builds the same graft (same player-attached {@code Storage}
 ...
 * existing {@code GraftScreenPresence} dispatch (screen-agnostic — it already
 * finds {@link MenuKitSlot}s in {@code screen.getMenu().slots}); shift-click into
 * them is {@link GraftQuickMove}; creative placement is the (now menu-general)
 * creative-set-slot bridge. A consumer that wants pockets on a chest registers a
 * projection source here <em>and</em> widens its {@code GraftScreenPresence}
 * targeting to that screen.
 */
public final class GraftProjection {

    private GraftProjection() {}

    /**
     * Builds (appends) a graft onto a matching foreign menu. Called on BOTH sides
     * for each opened menu the {@link #register registered} predicate accepts, so
     * it <b>must be deterministic</b> — same graft set, same slot order, same
     * storage size regardless of side. Typically just
     * {@code MenuKitGraft.onto(menu, player)....graft()} with the same parameters
    ...
    /**
     * Registers a projection source: project the graft built by {@code factory}
     ...
     * note in the class doc. Opt-in per graft: a graft that should stay
    ...
    /**
     * Appends every matching projected graft onto {@code menu}. Invoked by the
     ...
     */
    public static void appendProjectedGrafts(AbstractContainerMenu menu, Player player) {
